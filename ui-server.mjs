...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith('/ui-api/')) {
    ...
    const allowed =
      (req.method === 'GET' && ['/status', '/metadata', '/jobs', '/bulk/status', '/assistant/metadata/audit'].includes(path)) ||
      (req.method === 'POST' && ['/auth/start', '/bulk/start', '/bulk/pause', '/bulk/resume', '/bulk/cancel', '/selftest', '/assistant/chat', '/assistant/metadata/merge'].includes(path)) ||
