...
if (start < 0 || end < 0) throw new Error('Pipeline 13 patch failed: mergeMetadata function not found');
...
const replacement = [
  ...
  "    if (!sourceItem) { details.push({ sourceId, skipped: true, reason: 'not-found' }); continue; }",
  "    if (kind === 'tag' && sourceItem.is_inbox_tag) throw new Error('Inbox tags are protected and cannot be merged.');",
  "",
  ...
  "        tags.delete(Number(sourceItem.id));",
