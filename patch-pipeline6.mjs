...
replaceOnce(
  'technical provenance functions',
...
async function cleanupLegacyToolTags(taxonomy) {
  if (legacyToolTagsCleaned) return;
...
async function applyProvenanceTags(current, taxonomy, semanticTagIds, run) {
  await cleanupLegacyToolTags(taxonomy);
  const legacyIds = new Set(taxonomy.tags.filter(tag => provenancePrefix(tag.name)).map(tag => Number(tag.id)));
...
replaceOnce(
  'usage error helper before worker',
